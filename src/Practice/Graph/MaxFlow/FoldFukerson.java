...
public class FoldFukerson {
	...
	public FoldFukerson(FlowNetwork flowNetwork, Vertex s, Vertex t) {
		...
		while (hasAugmentingPath(flowNetwork, s, t)) {
			...
			for (Vertex v = t; v != s; v = edgeTo[v.getId()].getOther(v)) {
				minValue = Math.min(minValue,
						edgeTo[v.getId()].getResidualCapacity(v));
			}
	...
	public boolean inCut(int index){
		return marked[index] ;
	}
	...
	private boolean hasAugmentingPath(FlowNetwork flowNetwork, Vertex s,
			Vertex t) {
